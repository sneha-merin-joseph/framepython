...
def product_add(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            movie.save()
            return redirect('movie:product_detail', pk=movie.pk)
    else:
        form = ProductForm()
...
def product_update(request, pk):
    ...
    if request.method == 'POST':
        form = ProductForm(request.POST, request.FILES, instance=movie)
        if form.is_valid():
            movie = form.save(commit=False)
            movie.save()
            return redirect('movie:product_detail', pk=movie.pk)
    ...
    return render(request, 'User-Page/Movies/movie_update.html', {'form': form, 'movie': movie})
...
def product_delete(request, pk):
    ...
    return render(request, 'products/product_delete_confirm.html', {'movie': movie})
